...
public class DictionaryManagement {
  ...
  public void insertFromFile()  throws FileNotFoundException,IOException {
    Scanner tu;
    File file = new File("src/word/Tudien.txt");
    tu = new Scanner(file);
    BufferedReader reader = new BufferedReader(new FileReader("/home/quanghuy/BTL/src/word/Tudien.txt"));
    int lines = 0;
  ...
  public void dictionaryExportToFile() throws FileNotFoundException,IOException{
    //Path wFile = Paths.get("src/word/tudien.txt");
    for(int i=0; i<wordCount; i++){
      Files.write(Paths.get("src/word/tudien.txt"), (a.dictionary.get(i).word_target + "  " + a.dictionary.get(i).word_explain + System.lineSeparator()).getBytes(), StandardOpenOption.APPEND);
    }
  }
